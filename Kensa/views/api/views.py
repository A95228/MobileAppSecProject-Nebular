...
def api_user_permission(request):
    try:
        if request.path.startswith("api/v1/upload") or request.path.startswith(
            "api/v1/scan"
        ):
            return True
        ...
        try:
            ...
            if match:
                ...
                if malware_overview is not None:
                    return make_api_response(
                        {"security_score": malware_overview}, OK
                    )
                return make_api_response({"msg": "Not exist"}, OK)
            else:
                return make_api_response({"error": "HASH error"}, BAD_REQUEST)
...
class ComponentsActivities(RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            if system != "android":
                return make_api_response(
                    {"error": "This API only supports for Android"},
                    BAD_REQUEST,
                )
            if match:
...
class ReconEmailsView(RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if emails is None:
            return make_api_response(
                {"error": "no emails for %s" % md5}, status=NOT_FOUND
            )
...
class ReconURLsView(RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if urls is None:
            return make_api_response(
                {"error": "no urls for %s" % md5}, status=NOT_FOUND
            )
...
class ReconFirebasedbURLsView(RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if system == "android":
            firebase_urls = StaticAnalyzerAndroid.get_recon_firebase_db(
                md5, page
            )
        elif system == "ios":
        ...
        if firebase_urls is None:
            return make_api_response(
                {"error": "no firebase urls for %s" % md5}, status=NOT_FOUND
            )
...
class ReconStringsView(RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if strings is None:
            return make_api_response(
                {"error": "no strings for %s" % md5}, status=NOT_FOUND
            )
...
class ReconTrackersView(RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if trackers is None:
            return make_api_response(
                {"error": "no trackers for %s" % md5}, status=NOT_FOUND
            )
...
class GetDomainsDataView(RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            data = StaticAnalyzerAndroid.get_domains_data(request.GET["md5"])
        except:
            return make_api_response({"error": "contact sysadmin"}, status=500)

        if data is None:
            return make_api_response(
                {"error": "No data to preview"}, status=NOT_FOUND
            )
    ...
    def get(self, request, *args, **kwargs):
        ...
        if data is not None:
            if isinstance(data, dict):
                return make_api_response(data=data, status=OK)
            return JsonResponse(
                data=data, safe=False, status=OK
            )  # strange case
...
class GetSignerCertificateView(RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if data is None:
            return make_api_response(
                data={"info": "nothing found"}, status=NOT_FOUND
            )
        return make_api_response(data=data, status=OK)
...
class PDFReportView(RetrieveAPIView):  # working
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not match:
            return make_api_response(
                {"error": "Invalid scan hash"}, BAD_REQUEST
            )
