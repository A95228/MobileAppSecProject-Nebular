    ...
    try:
        ...
        if match and (filename.lower().endswith('.apk') or filename.lower().endswith('.zip')):
            ...
            if filename.lower().endswith('.apk'):
                ...
                if StaticAnalyzerAndroid.objects.filter(MD5=app_dic["md5"]).exists():
                ...
                if 'error' not in context and status == 200:
                    update_scan_timestamp(app_dic['md5'])
                    return context, status
                else:
                    logger.exception('Saving to Database Failed')
                    return {'error': "Saving to Database Failed"}, 500
                
                return {"error" : "Saving to Database Failed"}, 500
            
            # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            # Case file extension is zip.
            # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                ...
                if valid and (pro_type in ['eclipse', 'studio']):
                    ...
                    try:
                        ...
                        if 'error' not in context and status == 200:
                            ...
                            return {"error" : "Saving to Database Failed, no context"}, 500
                        
                        return {"error" : "Saving to Database Failed, no context"}, 500
                            

                    except Exception as error:
                        logger.error(str(error) + "line number 422")
                        logger.exception('Saving to Database Failed')
                    return {'error': 'Saving to Database Failed'}, 500
            ...
            error = 'Hash match failed or Invalid file extension or file type'
            logger.error(error)
            return {'error': error}, 500
